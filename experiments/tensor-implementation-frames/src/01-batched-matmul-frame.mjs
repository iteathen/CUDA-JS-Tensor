...
function analyzeMatmulReason(base, left, right, node) {
  const reasons = [...base];
...
function profile(node, left, right, reasons) {
  return Object.freeze({
    ...
    reasons,
    eligibleForCurrentSIMT: reasons.length === 0,
    eligibleForProposedCublasLtProfile: reasons.length === 0,
  });
}
...
export function frameBatchedMatmul(plan) {
  ...
  for (const node of plan.program.nodes) {
    ...
    const reasons = analyzeMatmulReason([], left, right, node);
  ...
  return frameRecord({
    ...
    plan,
    scope: 'Rank-3 contiguous f32 matmul with explicit finite batch broadcasting.',
    candidates,
    blockers: [
