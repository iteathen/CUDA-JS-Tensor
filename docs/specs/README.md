...
Accepted status authorizes only the bounded contracts written here. The four original public CUDA-JS mechanism prerequisites are available in the exact recorded compatibility revision; CUDA-JS-Tensor semantic, lifecycle, native, and performance evidence remains independently gated per component.
