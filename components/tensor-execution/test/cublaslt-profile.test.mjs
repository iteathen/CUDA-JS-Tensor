...
function fakePublicRuntime({
  openError = null,
  openCategory = 'unsupported',
  planError = null,
  planCategory = 'unsupported',
  prepareError = null,
  workspaceBytes = 256,
  ...
  const runtime = {
    observation,
    async describe() { return { package: { name: 'cuda-js', version: '0.1.0-alpha.18', publicApiSchema: 1 }, state: 'open', profile: 'tensor-cublaslt-test', device: { architecture: { class: 'compute_75' } } }; },
    ...
    async openCublasLt() {
      ...
      if (openError) throw codedError(openError, openCategory);
      return stateful('cublaslt-adapter', {
        profile: 'cublaslt-f32-row-major-matmul-v1',
        provider: Object.freeze({ name: 'cuBLASLt', version: 'mock-13.3', qualification: 'portable-contract-only', workspaceAlignmentBytes }),
        async createF32MatmulPlan(options) {
          observation.planCreates.push(options);
          if (planError) throw codedError(planError, planCategory);
...
test('preference exposes dtype, batch, layout, active-extent, derived-offset, and empty fallbacks', () => {
  ...
  for (const entry of cases) {
    ...
    assert.deepEqual(request.canonical.alignment, { operandByteOffset: 4 });
  }
});

test('explicit preference builds one mixed prepared DAG and derives provider workspace alignment', async () => {
  const runtime = fakePublicRuntime({ workspaceAlignmentBytes: 512 });
  ...
  assert.equal(resolved.canonical.backendProfile.provider.identity.workspaceAlignmentBytes, 512);
  assert.equal(resolved.canonical.backendProfile.matmuls[0].workspace.requiredByteOffsetAlignment, 512);
...
test('resolved plans own independent CUDA-JS borrowers while provider sharing stays lower-owned', async () => {
  ...
  assert.equal(runtime.observation.adapterOpens, 2);
...
test('preference follows unsupported category while strict and non-unsupported failures remain hard', async () => {
  const unavailable = fakePublicRuntime({ openError: 'SYNTHETIC_LOWER_UNSUPPORTED' });
  ...
  assert.deepEqual(preferred.canonical.backendProfile.matmuls[0].reasons, ['provider-unsupported']);
  await preferred.close();
  await assert.rejects(resolveWithFakeCompiler(unavailableSession, mixedProgram(), { backend: 'cublaslt' }), { code: 'SYNTHETIC_LOWER_UNSUPPORTED' });
  assert.equal((await unavailableSession.close()).graceful, true);

  const incompatible = fakePublicRuntime({ openError: 'CUBLASLT_PROVIDER_IDENTITY' });
  const incompatibleSession = await TensorSession.open({ runtime: incompatible, runtimeOwnership: 'owned' });
  const incompatibleFallback = await resolveWithFakeCompiler(incompatibleSession, mixedProgram(), { backend: 'prefer-cublaslt' });
  assert.equal(incompatibleFallback.backend, 'simt');
  assert.deepEqual(incompatibleFallback.canonical.backendProfile.matmuls[0].reasons, ['provider-unsupported']);
  await incompatibleFallback.close();
  assert.equal((await incompatibleSession.close()).graceful, true);

  const providerFailure = fakePublicRuntime({ openError: 'SYNTHETIC_PROVIDER_FAILURE', openCategory: 'provider' });
  const providerFailureSession = await TensorSession.open({ runtime: providerFailure, runtimeOwnership: 'owned' });
  await assert.rejects(resolveWithFakeCompiler(providerFailureSession, mixedProgram(), { backend: 'prefer-cublaslt' }), { code: 'SYNTHETIC_PROVIDER_FAILURE' });
  assert.equal((await providerFailureSession.close()).graceful, true);
});

test('plan preference fallback is category-based and post-plan preparation rollback closes every acquired child', async () => {
  const unavailablePlan = fakePublicRuntime({ planError: 'SYNTHETIC_PLAN_UNSUPPORTED' });
  const fallbackSession = await TensorSession.open({ runtime: unavailablePlan, runtimeOwnership: 'owned' });
  ...
  assert.deepEqual(fallback.canonical.backendProfile.matmuls[0].reasons, ['plan-unsupported']);
  assert.equal(unavailablePlan.observation.adapterCloses, 1);
  await fallback.close();
  assert.equal((await fallbackSession.close()).graceful, true);

  const hardPlanFailure = fakePublicRuntime({ planError: 'SYNTHETIC_PLAN_PROVIDER_FAILURE', planCategory: 'provider' });
  const hardPlanSession = await TensorSession.open({ runtime: hardPlanFailure, runtimeOwnership: 'owned' });
  await assert.rejects(resolveWithFakeCompiler(hardPlanSession, mixedProgram(), { backend: 'prefer-cublaslt' }), { code: 'SYNTHETIC_PLAN_PROVIDER_FAILURE' });
  assert.equal(hardPlanFailure.observation.adapterCloses, 1);
  assert.equal((await hardPlanSession.close()).graceful, true);
...
test('preference falls back at the resolved accelerator workspace-count gate while strict mode rejects', async () => {
  ...
  assert.equal(preferred.canonical.backendProfile.matmuls[0].reasons.includes('workspace-count-limit'), true);
  assert.equal(runtime.observation.planCloses, 1);
  assert.equal(runtime.observation.adapterCloses, 1);
  await preferred.close();
